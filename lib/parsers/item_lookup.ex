defmodule Emerald.Parsers.ItemLookup do
  ...
  defp item(doc) do
    %{
      ...
      availability: list_price(doc).availability,
      condition: list_price(doc).condition,
      currency_code: list_price(doc).currency_code,
      prime: list_price(doc).prime,
      super_saver: list_price(doc).super_saver,
      price: list_price(doc).amount
      # title: title(doc),
  ...
  defp throttled?(doc) do
    xpath(doc, ~x"/ItemLookupErrorResponse/Error/Code/text()") == 'RequestThrottled'
  end

  defp error_message(doc) do
    xpath(doc, ~x"/ItemLookupErrorResponse/Error/Message/text()")
  end
